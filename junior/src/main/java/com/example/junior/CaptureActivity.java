...
public class CaptureActivity extends AppCompatActivity implements View.OnClickListener, View.OnLongClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        tv_capture = findViewById(R.id.tv_capture);             //  从布局文件中获取名叫tv_capture的文本视图
        iv_capture = findViewById(R.id.iv_capture);             //  从布局文件中获取名叫iv_capture的图像视图
        tv_capture.setDrawingCacheEnabled(true);                //  开启文本视图tv_capture的绘图缓存
        Button btn_chat = findViewById(R.id.btn_chat);          //  从布局文件中获取名叫btn_chat的按钮
        Button btn_capture = findViewById(R.id.btn_capture);    //  从布局文件中获取名叫btn_capture的按钮
        btn_chat.setOnClickListener(this);                      //  给btn_chat设置点击监听器
        btn_chat.setOnLongClickListener(this);                  //  给btn_chat设置长按监听器
        btn_capture.setOnClickListener(this);                   //  给btn_capture设置点击监听器
    }
    ...
    public boolean onLongClick(View v) {
        if (v.getId() == R.id.btn_chat) {                       //  长按了按钮btn_chat，则清空文本视图tv_capture
            tv_capture.setText("");
        }
        return true;
    }
    ...
    public void onClick(View v) {                               //  一旦监听到点击动作，就触发监听器的onClick方法
        if (v.getId() == R.id.btn_chat) {                       //  点击了聊天按钮，则给文本视图添加聊天文字
            ...
            Bitmap bitmap = tv_capture.getDrawingCache();       //  从文本视图tv_capture的绘图缓存中获取位图对象
            iv_capture.setImageBitmap(bitmap);                  //  给图像视图iv_capture设置位图对象
    ...
    private Runnable mResetCache = new Runnable() {
        @Override
        public void run() {
            tv_capture.setDrawingCacheEnabled(false);           //  关闭文本视图tv_capture的绘图缓存
            tv_capture.setDrawingCacheEnabled(true);            //  开启文本视图tv_capture的绘图缓存
        }
    };
}
