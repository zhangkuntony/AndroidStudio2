...
public class SpinnerIconActivity extends AppCompatActivity {
    ...
    private void initSpinnerForSimpleAdapter() {
        List<Map<String, Object>> list = new ArrayList<Map<String, Object>>();              //  声明一个映射对象的队列，用于保存行星的图标与名称配对信息
        ...
        starAdapter.setDropDownViewResource(R.layout.item_simple);          //  设置简单适配器的布局样式
        Spinner sp = findViewById(R.id.sp_icon);                            //  从布局文件中获取名叫sp_icon的下拉框
        sp.setPrompt("请选择行星");                                          //  设置下拉框的标题
        sp.setAdapter(starAdapter);                                         //  设置下拉框的简单适配器
        sp.setSelection(0);                                                 //  设置下拉框默认显示第一项
        sp.setOnItemSelectedListener(new MySelectedListener());             //  给下拉框设置选择监听器，一旦用户选中某一项，就触发监听器的onItemSelected方法
    }
